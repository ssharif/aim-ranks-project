...
\usepackage{amssymb,amsmath,mathrsfs,amsthm,amscd,hyperref,amsfonts}
...
\label{smallerfldthm}
\[
\rank_\Z J(\F_q(t^{1/d})) = (r-1)\left[\sum_{e\mid d}\frac{\varphi(e)}{o_q(e)}-
\frac{2}{o_q(r)} \right].
\]
\end{thm}

Therefore, over (fixed) $\F_q(u)$, Jacobians of curves of genus $r-1$ have unbounded rank.

This is for $r$ and $p$ such that $r \mid (p^f+1)$ for some $f$, 
i.e., such that the order of $p$ mod $r$ is even, 
i.e., $-1 \in \langle p \rangle \subset (\Z/r\Z)^\ast$. 
...
\section{The $r$-descent}
%Authors: Lisa Berger, Ren\'e Pannekoek.

[Ren\'e will supply a new version.]

Let $q$ be an odd prime power. Let $K_1$ be the field $\F_q(t)$ with $t$ indeterminate, and for $d>1$ define $K_d = K_1(\mu_d,t^{1/d})$, denoting $u = t^{1/d}$. The field extension $K_1(\mu_d) \subset K_d$ is cyclic of degree $d$. Let $\nu$ be an integer and henceforth choose $d$ to be of the form $q^\nu+1$. (Note: I choose $\nu$ instead of $f$, since the variable name isn't often used and I have a better use for the letter $f$ in mind. Also, I think it's practical to use the letter $K$ not for $K_1$, but for the varying field $K_d$.)

Let $r$ be an odd prime dividing $d$. Consider the smooth projective curve $C'$ over $K_1$ with affine model
$$
\eta^r = \xi^{r-1}(\xi+1)(\xi+t).
$$
The model above is singular, but it has the good property that for each (singular) point on it, there is only one point above it on the normalization. The same property still holds when passing to the projective closure of the model in $\p^2$. Applying Riemann--Hurwitz to the cover $C' \rightarrow \p^1$ determined by the function $\xi$, we get:
$$
\textnormal{genus}(C') = r - 1.
$$
Since the Poonen-Schaefer paper requires the curve to be
unramified at infinity,
we perform the change of coordinates
$$
x = \frac{1}{\xi - 1},~~~y = x^2 \eta = \frac{\eta}{(\xi - 1)^2},
$$
and call the resulting curve $C$ (again over $K_1$):
$$
C : y^r = x^{r-1}(x + 1)^{r-1}(2x + 1)((t+1)x + 1),
$$
whose RHS we denote by $f(x)$. By $f_0(x)$ we denote the ``reduced polynomial'' $x(x + 1)(2x + 1)((t+1)x + 1)$. The curve $C$ has an automorphism $\zeta_r$ defined by $\zeta_r(x,y) = (x,\zeta_r y)$. There is a projection $\pi$ to the $x$-axis. We set $J := \textnormal{Jac}(C)$; $J$ has an automorphism induced by $\zeta_r$ which we denote by the same symbol, and we set $\phi := 1 - \zeta_r$. 

We fix some $\nu > 0$, and abbreviate $K := K_d$. Let $L$ be the $K$-algebra $K[T]/f_0(T)$, which by the Chinese remainder theorem we may identify with $K^4$ by ``evaluation in the roots of $f_0$'':
\begin{equation}\label{crt}
h \in L \, \mapsto \, (h(0),h(-1),h(-1/2),h(-1/(t+1))).
\end{equation}
$L$ has a ``weighted'' norm $\Nm$ defined by
$$
\Nm((\alpha_0,\alpha_1,\alpha_2,\alpha_3)) = \alpha_0^{r-1}\alpha_1^{r-1}\alpha_2\alpha_3.
$$
By ``Poonen--Schaefer theory'', we have a homomorphism called $(x-T)$
$$
(x-T) \, : \, \Pic(C_K) \rightarrow L^{\ast}/L^{\ast r}K^{\ast} 
$$
whose codomain can, using (\ref{crt}), also be written as
$$
L^{\ast}/L^{\ast r}K^{\ast}  =  \left( K^{\ast}/K^{\ast r} \right)^4 / K^{\ast},
$$
where $K^{\ast}$ sits diagonally inside $\left( K^{\ast}/K^{\ast r} \right)^4$. Therefore\footnote{Here and after, I'm using that $J(K) = \textnormal{Pic}^0(C_K)$, since our $C$ evidently has points over the ground field $K_1$.} by restriction we obtain the following map (also called $(x-T)$):
$$
(x-T) \, : \, J(K) \rightarrow L^{\ast}/L^{\ast r}K^{\ast} 
$$
which send the class of a divisor $D = \sum_{P \in C_K} n_P \cdot P \in \textnormal{Div}(C_K)$ whose support is disjoint from the points above $\infty$ and the ramification points of $C_K \rightarrow \p^1_K$ to $\prod_{P \in C_K} (x(P)-T)^{n_P}$. (By P--S, section 5 this actually defines $(x-T)$ on the whole of $\Pic(C_K)$.) By P--S Theorem 11.3, the kernel of the restriction of $(x-T)$ to $J(K)$ is precisely $\phi(J(K))$. Put differently, $(x-T)$ induces an injection
\begin{equation}
\label{injectie}
J(K)/\phi(J(K)) \hookrightarrow L^{\ast}/L^{\ast r}K^{\ast}.
\end{equation}

\begin{rem}\label{normmooi}
\upshape
Note that $\Nm : L \rightarrow K$ induces a homomorphism
$$
L^{\ast}/L^{\ast r}K^{\ast} \rightarrow K^{\ast}/K^{\ast r}
$$
which in the present case admits the description (since $\alpha^{r-1} = \alpha^{-1}$ in $K^{\ast}/K^{\ast r}$):
$$
(\alpha_0,\alpha_1,\alpha_2,\alpha_3) \mapsto \alpha_0^{-1}\alpha_1^{-1}\alpha_2\alpha_3.
$$
\end{rem}
\noindent
\textbf{Example.} Let's see where the class of $P_0 := (\frac{1}{u-1},\frac{u(u+1)^{d/r}}{(u-1)^2})$ maps to. We get $(x-T)(P_0) = (\frac{1}{u-1} , \frac{1}{u-1} + 1, \frac{1}{u-1} + \frac{1}{2}, \frac{1}{u-1} + \frac{1}{t+1})$ which is the same element as $(1,u,\frac{1}{2}(u+1),\frac{1}{t+1}(u+t))$ (we may multiply all coordinates by the same element of $K^{\ast}$). (The images of the $\Gal(K_d/K_1(\mu_d))$-conjugates of $[P_0]$ can be determined from the $\Gal(K_d/K_1)$-equivariance of $(x-T)$.) Note that the norm of this element is $\frac{1}{2(t+1)}$. This leads to:
\begin{lem}\label{NormLemma}
Let $D \in \Pic(C_K)$ be a divisor of degree 1. Then $(x-T)([D])$ has norm $\frac{1}{2(t+1)}$.
\end{lem}
\begin{proof}
The divisor $D' = D - P_0$ has degree zero and so by P--S 12.1 the image of its class has norm 1. But $\Nm : L \rightarrow K$ induces a homomorphism $L^{\ast}/L^{\ast r}K^{\ast} \rightarrow K^{\ast}/K^{\ast r}$, so the norms of $(x-T)([D])$ and $(x-T)([P_0])$ must be the same.
\end{proof}
The ramification points of $\pi_K : C_K \rightarrow \p^1_K$ are
$$
Q_{\infty} = (0,0), ~~~ Q_0 = (-1,0), ~~~ Q_1 = (-1/2,0), ~~~ Q_t = (-1/(t+1),0).
$$
We now determine the image of the corresponding divisor classes under $(x-T)$, using a lemma that suggests itself immediately when doing descent calculations. Note that, if $I \subset \{0,1,2,3 \}$ is some (possibly empty) subset, by $\pr_I : L \rightarrow \prod_{I} K$ we mean the projection map that takes an element of $L = K^4$ and ``forgets'' the coordinates with indices outside the set $I$. Furthermore, this projection map induces the horizontal maps in the following commutative diagram:
\[
\xymatrix{ 
L^{\ast} \ar[r]^{\pr_I} \ar[d]^{\can} & \prod_{I} K^{\ast} \ar[d]^{\can}  \\\
L^{\ast}/L^{\ast r}K^{\ast} \ar[r]^{\pr_I}  & \left( \prod_{I} K^{\ast}/K^{\ast r} \right)/K^{\ast} } 
\]
Here the vertical maps are just the canonical maps, and $K^{\ast}$ sits diagonally inside $\prod_I K^{\ast}$.
\begin{lem}\label{ronald}
Let $D = \sum_{P \in C_K} n_P \cdot P \in \textnormal{Div}(C_K)$ be any effective divisor with support disjoint from $\pi_K^{\ast} \infty$ and let $h = \prod_{P \in C_K} (x(P)-T)^{n_P} \in L$. Let $\pr_I$ be a projection $\pr_I : L \rightarrow \prod_I K$ for which $\pr_I(h)$ is a unit. Then the class of $\pr_I(h)$ in $\left( \prod_{I} K^{\ast}/K^{\ast r} \right)/K^{\ast}$ coincides with $\pr_I((x-T)([D]))$.
\end{lem}
This means exactly that, even though the support of $D$ may contain problematic points (but not those at infinity), we can evaluate $(x-T)([D])$ in the usual way on all the coordinates of $K^4$ where the output makes sense (i.e. is non-zero).
\begin{proof}
We have to show that, if $\psi \in \kappa(C_K)$ is such that the support of $D' = D + \textnormal{div}(\psi)$ does not contain points of $\pi_K^{\ast} \infty$ and the ramification points of $\pi_K$ (such a $\psi$ exists), then we have:
$$
\pr_I((x-T)([D']) = \pr_I(h).
$$
This is a straightforward application of the Weil reciprocity theorem.
\end{proof}
By the preceding lemma, we can calculate for each of the points $Q_{\bullet}$ the image under $(x-T)$, save for one coordinate:
\begin{center}
$(x-T)([Q_{\infty}]) = (\,?\,,1,\frac{1}{2},\frac{1}{t+1}),~~~(x-T)([Q_{0}]) = (-1,\,?\, , -\frac{1}{2},-\frac{t}{t+1}),$\\
$(x-T)([Q_{1}]) = (-\frac{1}{2},\frac{1}{2},\,?\, , - \frac{t-1}{2(t+1)}),~~~(x-T)([Q_{t}]) = (-\frac{1}{t+1},\frac{t}{t+1} , \frac{t-1}{2(t+1)}, \, ? \,).$
\end{center}
Tidying up denominators and minus signs (keeping in mind that $-1 \in K^{\ast r}$, which follows from the fact that $K$ contains $\F_q(\mu_d) = \F_{q^{2 \nu}}$, which contains a $(q^{2 \nu}-1)$-st = $d(q^{\nu}-1)$-st root of unity, and therefore a $2r$-th root of unity):
\begin{center}
$(x-T)([Q_{\infty}]) = (\,?\,,1,\frac{1}{2},\frac{1}{t+1}),~~~(x-T)([Q_{0}]) = (1,\,?\, , \frac{1}{2},\frac{t}{t+1}),$\\
$(x-T)([Q_{1}]) = (1,1, \,?\, , \frac{t-1}{t+1}),~~~(x-T)([Q_{t}]) = (1,t, \frac{t-1}{2}, \, ? \,).$
\end{center}
Using Remark \ref{normmooi} and Lemma \ref{NormLemma} we quickly fill in the question marks:
\begin{center}
$(x-T)([Q_{\infty}]) = (1,1,\frac{1}{2},\frac{1}{t+1}),~~~(x-T)([Q_{0}]) = (1,t, \frac{1}{2},\frac{t}{t+1}),$\\
$(x-T)([Q_{1}]) = (1,1, \frac{1}{2(t-1)} , \frac{t-1}{t+1}),~~~(x-T)([Q_{t}]) = (1,t, \frac{t-1}{2}, \frac{t}{(t-1)(t+1)}).$
\end{center}
We see the relation:
$$
(r-1)Q_0 + Q_1 + Q_t ~\sim~ (r+1)Q_{\infty}
$$
(which on $C'$ can be easily seen by considering the principal divisor associated to the function $\eta$) reappear.

We next compute the images of the following $d+3$ elements of $J(K) = \textnormal{Pic}^0(C_K)$:
\begin{center}
$R_0 = [Q_0] - [Q_{\infty}],~~~R_1 = [Q_1] - [Q_{\infty}], ~~~R_t = [Q_t] - [Q_{\infty}],$\\
$P_i = \left[(\frac{1}{\zeta^i u - 1},\frac{\zeta^i u(\zeta^i u+1)^{d/r}}{(\zeta^i u - 1)^2}) \right] - [Q_{\infty}],$
\end{center}
where $i \in \{0,1,\ldots,d-1\}$ and $\zeta = \zeta_d$ is a $d$-th primitive root of unity:
\begin{align*}
& (x-T)(R_0) = (1,t,1,t) = (1,1,1,1);\\
& (x-T)(R_1) = (1,1,\frac{1}{t-1},t-1); (x-T)(R_t) = (1,1,t-1,\frac{1}{t-1}) \\
& (x-T)(P_i) = (1,\zeta^i u,\zeta^i u + 1,\zeta^i u + t);
\end{align*}
At this point, the calculations start reminding us again of the work done at AIM. From the first computation and the injectivity of (\ref{injectie}) we see that:
$$
R_0 = (1-\zeta_r)(R_0')
$$
for some $R_0' \in J(K)$.

Consider the $\F_r$-linear map (mapping row vectors to row vectors, say)
$$
M : \left( K^{\ast}/K^{\ast r} \right)^4 / K^{\ast} \rightarrow \F_r^{d+1}
$$
defined by
\small
$$
M(\alpha_1,\alpha_2,\alpha_3,\alpha_4) = (\textnormal{ord}_u(\alpha_2/\alpha_1),\textnormal{ord}_{u+ 1}(\alpha_3/\alpha_1), \textnormal{ord}_{\zeta u+ 1}(\alpha_3/\alpha_1) , \ldots ,\textnormal{ord}_{\zeta^{d-1} u+ 1}(\alpha_3/\alpha_1))
$$
\normalsize
To determine the dimension of the subspace $V$ of the $\F_r$-vector space $\left( K^{\ast}/K^{\ast r} \right)^4 / K^{\ast}$ spanned by the elements $a = (1,1,\frac{1}{t-1},t-1)$ and $b_i = (1,\zeta^i u,\zeta^i u + 1,\zeta^i u + t)$, we look at their images under $M$. The matrix containing the images of the $a$ and $b_i$ is:
$$
\left( \begin{array}{c|ccccccc}
a & 0 & -1 & -1 & -1 & \ldots & -1 & -1 \\
b_0 & 1 & 1 & 0 & 0 & \ldots & 0 & 0 \\
b_1 & 1 & 0 & 1 & 0 & \ldots & 0 & 0 \\
b_2 & 1 & 0 & 0 & 1 & \ldots & 0 & 0 \\
\ldots & \ldots & \ldots & \ldots & \ldots & \ldots & \ldots & \ldots \\
b_{d-2} & 1 & 0 & 0 & 0 & \ldots & 1 & 0 \\
b_{d-1} & 1 & 0 & 0 & 0 & \ldots & 0 & 1
\end{array}
\right)
$$
This $(d+1) \times (d+1)$ matrix does not have full rank, since the inner product of each column with $(1,1,1,\ldots,1)$ is zero in $\F_r$ (this uses again that $r \mid d$). On the other hand, the last $d$ rows are certainly linearly independent. This proves that $V = \left\langle a, b_0, b_1, \ldots, b_{d-1} \right\rangle$ has dimension $\geq d$. We can prove equality if we can prove that the kernel of $\left.M \right|_V$ is trivial, which comes down to showing that
$$
a + b_0 + b_1 + \ldots + b_{d-1} = 0 \in L^{\ast}/L^{\ast r} K^{\ast},
$$
which is indeed the case, as a straightforward calculation shows.

The following lemma just summarizes the algebra that is needed for finishing the rank calculation, which is settled immediately afterwards:
\begin{prop}
Let $r$ be a prime, $\zeta = \zeta_r$ be a primitive $r$th root of unity, and $R = \Z[\zeta]$. Let $M,N$ be $R$-modules such that $M$ is finitely generated and $\textnormal{Ann}(N) = (1-\zeta)R$ (i.e., $N$ has the structure of an $\F_r$-vector space). Suppose that we have an exact sequence of $R$-modules:
\begin{equation}\label{exact}
M \stackrel{1- \zeta}{\rightarrow} M \stackrel{g}{\rightarrow} N,
\end{equation}
and suppose that $M' \subset M$ is an $R$-submodule. Then we have:
$$
\textnormal{rank}_{R}(M') \geq \dim_{\F_r} g(M') - \dim_{\F_r} M[1-\zeta].
$$
Moreover,
$$
\textnormal{rank}_{\Z}(M') = (r-1) \cdot \textnormal{rank}_{R}(M').
$$
\end{prop} 
\begin{proof}
Observe that $M[1-\zeta] \subset M[r]$, since $1 - \zeta$ divides $r$ in $R$. Without loss of generality assume $M[r] \subset M'$ (replacing $M'$ by $M' + M[r]$ possibly increases $\dim_{\F_r} g(M')$, but leaves $\textnormal{rank}_{R}(M')$ and $\dim_{\F_r} M[1-\zeta]$ unchanged).
Letting $R' = R_{(1-\zeta)}$, we pass to the localizations at $(1-\zeta)$:
\begin{equation}
\label{diag}
\xymatrix{ 
M             \ar[r]^{1-\zeta} \ar[d] & M             \ar[r]^{g} \ar[d]             & N  \ar[d]^{\textnormal{id}}       \\\
M_{(1-\zeta)} \ar[r]^{1-\zeta}        & M_{(1-\zeta)} \ar[r]^{g_{(1-\zeta)}}        & N
} 
\end{equation}
Since localization is exact, the bottom maps still form an exact sequence. We let $M'' = M' \otimes_R R' \subset M \otimes_R R' = M_{(1-\zeta)}$, and choose an identification
$$
M'' = M''_{\textnormal{tor}} \oplus R_{(1-\zeta)}^{\rho},
$$
where $\rho = \textnormal{rank}_R(M') = \textnormal{rank}_{R'}(M'')$. Now by exactness of the bottom row of (\ref{diag}) and in view of $R_{(1-\zeta)} / (1-\zeta) R_{(1-\zeta)} = \F_r$, the restriction of $g_{(1-\zeta)}$ to $M''$ factors via the quotient map
\begin{equation}
\label{voorbereiding}
M'' \rightarrow M''_{\textnormal{tor}} / (1-\zeta)M''_{\textnormal{tor}} \oplus \F_r^{\rho}
\end{equation}
%and therefore through
%$$
%A \rightarrow A_0/rA_0 \oplus A_2/rA_2.
%$$
(Since $m \in M''_{\textnormal{tor}} \Leftrightarrow (1-\zeta)m \in M''_{\textnormal{tor}}$ we have $M''_{\textnormal{tor}} / (M''_{\textnormal{tor}} \cap (1-\zeta)M'') = M''_{\textnormal{tor}}/(1-\zeta)M''_{\textnormal{tor}}$.) From (\ref{voorbereiding}) we infer an upper bound for the $\F_r$-dimension of $g(M')$:
$$
\dim_{\F_r} g(M') = \dim_{\F_r} g_{(1-\zeta)}(M'') \leq \rho + \log_r \# \left( M''_{\textnormal{tor}}/(1-\zeta)M''_{\textnormal{tor}} \right).
$$
The first inequality now follows from:
$$
\# \left(  \frac{ M''_{\textnormal{tor}} }{ (1-\zeta)M''_{\textnormal{tor}} } \right) = \frac{\# M''_{\textnormal{tor}} }{\# (1-\zeta)M''_{\textnormal{tor}}} = \# (M''_{\textnormal{tor}})[1-\zeta] =  \# M[1-\zeta].
$$
The second inequality follows from the first one, and the fact that the $\Z$-algebra $R$ has (constant) rank $r-1$ as a $\Z$-module.
%But now
%$$
%\# g(A_0) = \# \left( A_0/(A_0 \cap \phi(A)) \right) \leq \# \left( A_0/\phi(A_0) \right) = \frac{\# A_0}{\# \phi(A_0)} = \# A[\phi],
%$$
%(the first equality uses the exactness of (\ref{exact})) and since both $g(A_0)$ and $A[\phi]$ are killed by $r$ it follows that:
%$$
%\dim_{\F_r} g(A) \leq \rank(A_2) + \dim_{\F_r} A[\phi].
%$$
\end{proof}
\noindent
We apply the Proposition to our case, with:\\
- $M = J(K)$, which has a natural $\Z[\zeta_r]$-structure by the presence of the automorphism $\zeta_r$ of $C$;\\
- $M' = \textnormal{the $R$-submodule generated by $P_0 , P_1 , \ldots , P_{d-1}$}$;\\
- $N = L^{\ast} / L^{\ast r} K^{\ast}$, and\\
- $g = (x-T)$.\\
We find the following
\begin{thm}
$$
\rank_{\Z}( \left\langle  P_0 , P_1 , \ldots , P_{d-1} \right\rangle ) \geq 
(r-1)(d-2) 
$$
\end{thm}
\begin{proof}
To use the previous proposition, we need to determine the quantities $\dim_{\F_r} g(M')$ and $\dim_{\F_r} M[1-\zeta]$. First off, $\dim_{\F_r} g(M')$ was previously known as $\dim_{\F_r}(V) = d$, in fact this computation occupied a large portion of our work. Also, $\dim_{\F_r} M[1-\zeta] = \dim_{\F_r} J(K)[\phi]$ is seen to be 2, as follows. Observe that the kernels of the $r-1$ isogenies $(1-\zeta_r^i)$ are all equal ($i = 1, \ldots, r-1$). Their composition is quickly seen to be $[r]$, since, if we let $\Phi_r(z)$ denote the $r$th cyclotomic polynomial $z^{r-1} + z^{r-2} + \ldots + 1$:
$$
\prod_{i=1}^{r-1} (1-\zeta_r^i) = \Phi_r(1) = r \in R,
$$
so has degree $r^{2g} = r^{2(r-1)}$. Hence, the $1-\zeta_r^i$ are all separable and of the same degree $r^2$. Since the isogeny $[r]$ factors via each of them, their kernels are isomorphic to $(\Z/r\Z)^2$.

Hence we get, substituting these values in the inequality from the previous proposition:
...
%Further to do: (1) find the missing $(1-\zeta_r)^2$-torsion point $R_0'$ (and pray that it is not inside $\ker((x-T))$), (2) upper-bound the rank using the Selmer group (perhaps explicitly construct $J[\phi]$-torsors over $C_K$ representing elements of the Selmer group) and (3) find out in which way using the automorphism $\zeta_r$ will increase the lower bound on the rank.


%\section{Ranks}
%
%[Below  just restates some statements given at the beginning. Proofs
%will be given.]
%
%\begin{thm}
%%\label{rkthm}
...
This can be combined with Theorem 1.4 of \cite{Cornelissen} 
to show that $J(K_d)[2]=0$ for $d$....
...
action of $\mu_r$ on $\XX$ which is given in coordinates by
...
of $\mu_r$ (or the order of vanishing at points where $x=y=0$) shows
