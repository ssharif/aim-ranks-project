...
$$f_0,\dots,f_{d-1},d_2,e_3,\dots,e_{r-1},e_1,e_2,d_3,\dots,d_{r-1}$$
...
\dots&(-\frac{r^2}2,-\frac{r(r-3)}2,\dots.-\frac{r(r-3)}2,-\frac{r(r-4)}2).
}$$
...
Arranging the rows in the order
$$f_0,\dots,f_{d-1},d_2,e_3,\dots,e_{r-1},e_2,e_1,d_3,\dots,d_{r-1}$$
yields a matrix in row echelon form and with the property that the
leading entry of each row divides every entry to the right.  Looking
at the leading terms then reveals that the invariant factors are $1$
...
+2\sum_{j=1}^{r-2}\sum_{k=r-j}^{r-1}\sum_{i\equiv k\mod
  r}\sigma^i\tau^j
\end{multline*}
...
The proof in the case $r$ is even is similar and will be left as an exercise
for the editor.
\end{proof}
...
Note that in $R^0/I^0$ we have 
$$\sum_{i=0}^{d-1}\sigma^i=\tau \sum_{i=0}^{d-1}\sigma^i
=\tau^2 \sum_{i=0}^{d-1}\sigma^i=\cdots$$
so that
$r\sum_{i=0}^{d-1}\sigma^i=(1+\tau+\cdots+\tau^{r-1})\sum_{i=0}^{d-1}\sigma^i=0$, 
and so $\sum_{i=0}^{d-1}\sigma^i$ itself is zero.  Similarly 
$\sum_{j=0}^{r-1}s_j\tau^{r-j}=0$ in $R^0/I^0$.
...
if $b$ is any lift of $\overline b$ to $R$, then we have
...
$$\left\langle\rho(\overline{a}),\rho(\overline{b})\right\rangle=
(d-1)\left\langle a(P_{00}),b(P_{00})\right\rangle.$$
Here the right hand pairing is the height pairing on $J_r(K_d)$
\end{prop}
