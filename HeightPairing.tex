...
Fix $0 \leq k \leq d-1$. Since the extension $K(u)$ is unramified over $t = 1$, the special fibers at $u = \zeta^k$ are each identical to that of $C/K$ when $t = 1$. Recall that the initial chart is $\caff$, with equation 
\[
(x_0 - 1) \bar{y}^r = x_0(x_0 + (u^d - 1))
\]
...
\begin{proof}
  ...
  Now suppose that $\zeta^{i+k} = -1$. We consider the chart $(0,r)_b$ from section \S~???. This latter chart was given by coordinates $x_0, b_0, \tau_0$, where $\tau_0$ was written in terms of $t_0$. Our original chart essentially substitutes $u_k$ for $t_0$, so we will define $\mu_k$ as the analogue of $\tau_0$; namely,
  \[
  \mu_k = \frac{u_k}{x_0}.
  \]
  Then
  ...
  \]
  If $d = r$, on the special fiber we obtain the point $(0, \omega^j, -\zeta^k)$. If $d > r$, we obtain $(0, 0, -\zeta^k)$, which again lies on the smooth locus of $E_1$.
\end{proof}
\end{document}
