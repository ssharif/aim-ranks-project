...
\begin{itemize}
    ...
    \item $E_{i,0}: y = x_i^2 + x_it_i + 1 = 0$ with multiplicity $i$.
\end{itemize}
...
When $k = 2$, the components $F_{i,2}$ and $E_{i,2}$ are the same as above, but $E_{i+1,2}$ has equation $x_i = 1+\tau_i+b_i^2 = 0$ with multiplicity $i+1$. The intersection $E_{i,2} \cap E_{i+1,2}$ is the same, but now $F_{i,2}$ intersects $E_{i+1,2}$ transversally at two points: $(x_i, b_i, \tau_i) = (0, \pm i, 0)$.

Independent of the value of $k$, one verifies that this chart is regular by computing the $\tau_i$-derivative.

The last chart, which we call $(i,k)_u$, is obtained via $x_i = u_it_i$, $y = v_it_i$, giving the model
\[
(u_iv_i^it_i^{i+1} - 1) v_i^k t_i^{k-2} = u_i(u_i + 1).
\]
The central fiber is given by $t_0 = t_i^{i+1}v_i^i$. The special fiber when $k \geq 3$ is given by
\begin{itemize}
    \item $D_{i+1,k}: t_i = u_i = 0$ with multiplicity $i+1$;
    \item $E_{i+1,k}: t_i = u_i + 1 = 0$ with multiplicity $i+1$;
    \item $D_{i,k}: v_i = u_i = 0$ with multiplicity $i$; and
...
The components $D_{i+1,k}$ and $D_{i,k}$ (when the latter exists) intersect transversally once at $(u_i, v_i, t_i) = (0,0,0)$, and the components $E_{i+1,k}$ and $E_{i,k}$ intersect transversally at $(-1,0,0)$. There are no other intersection points.

When $k = 2$, the components $V$ and $E$ are unchanged. There is no component $U'$, and $D_{i+1,2}$ has equation $t_i = u_i^2 + u_i + v_i^2 = 0$ with the same multiplicity. It intersects each of $D_{i,2}, E_{i,2}$ once transversally.
