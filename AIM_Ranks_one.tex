...
\end{itemize}
Note that if $i = 0$, $B_{i,k}$ has multiplicity $0$; this actually means that the component does not exist in this case. Otherwise,
the components $B_{i,k}$ and $X'_{i,k}$ cross transversally at $(x_i, b_i, \tau_i) = (0, 0, -1)$. The component $T'_{i,k}$ does not intersect the others regardless of the value of $i$.
...
The components $U_{i,k}$ and $V_{i,k}$ (when the latter exists) intersect transversally once at $(u_i, v_i, t_i) = (0,0,0)$, and the components $U'_{i,k}$ and $V'_{i,k}$ intersect transversally at $(-1,0,0)$. There are no other intersection points.
...
If $k \geq 3$, then
\begin{itemize}
    \item All of the $T$ and $T'$ are the same component for every subscript.
    \item $X_{i,k} = V_{i,k} = U_{i-1,k}$.
    \item $X'_{i,k} = U'_{i,k}$.
    \item $Y_{i,k} = B_{i,k} = V'_{i,k} = X'_{i-1,k} = U'_{i-1,k}$.
\end{itemize}
