...
\]
with central fiber given by $t_iy^i = 0$. Observe that if $i = 0$, this is exactly $\caff$. One verifies that chart $(i)$ is not regular at $(x_i, y, t_i) = 0$ if and only if $i < \lfloor r/2 \rfloor$ (or equivalently $r - 2i \geq 2$). As we will see, for such $i$ we will obtain chart $(i+1)$ by blowing-up.
...
(x_0 - 1) y^r = x_0^2.
\]
The fiber is irreducible with multiplicity $1$, and we call it $F_0$.
