...
\begin{proposition}\label{prop:block-Am-determinant}
  ...
  \end{array}
  \]
  where each $d_i$ is a positive integer, $A_{d_{1}-1}$ is the corresponding block matrix given in \Cref~\ref{prop:am-defn-det}, and $e_{d_{i}-1}$ is the standard column vector of length $d_i-1$ with a 1 in the last spot. Then
  \[
  \det (-B) = (\prod d_i)\bigg(\sum \frac{1}{d_1}\bigg).
  \]
\end{proposition}

\begin{proof}
  Let $\Gamma$ be the graph consisting of two vertices connected by $r$ paths, where the $i$th path consists of $d_i$ edges. Let $X$ be a curve over any strictly local ring with semi-stable reduction such that the dual graph of the special fiber of $X$ is $\Gamma$.\footnote{The existence of such an $X$ is proven in ???, but it will turn out to be irrelevant for this argument.} One checks that $B$ is the submatrix obtained by deleting a single row and column from the intersection matrix for the special fiber of $X$, and thus $\det (-B)$ equals the order of the component group associated to $X$. By~\cite[Prop.~9.6.10]{blr}, the order of the component group is precisely
  \[
  (\prod d_i)\bigg(\sum \frac{1}{d_1}\bigg).
  \]
  The claim follows.
\end{proof}
...
\end{array}\right]
  \]
  Notice that the submatrix obtained by deleting the first row and column from the above matrix is precisely $B'$. Therefore $\det (-B')$ is equal to the order of the component group of $C'$. But by \cite[Prop.~9.6.10]{blr}, this latter is
  \[
  (d-1)d^{r-1}\left(\frac{1}{d-1} + \frac{r-1}{d}\right) = d^{r-2}(rd-r+1).
  \]
  Now we apply Proposition~\ref{prop:dp-dot-p-cofactor}. We see that $\delta = 1$ since all components are reduced in the special fiber, and again by \cite[Prop.~9.6.10]{blr}, we have
  \[
