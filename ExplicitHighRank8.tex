...
In the figure, the $\ast$ denotes a triple intersection point. The component $F$ has multiplicity 1, and the components $D_i, E_i$ have multiplicity $i$.
  \item If $r$ is even, let $s = r/2$.  Then the dual graph\footnote{This is an abuse of terminology, as the reduction has a three-fold intersection point.} of $C_k$ is given by Figure~\ref{fig:superelliptic-dual-graph-even}.
\begin{figure}[h]\centering
\[
\xygraph{
  !{<0cm,0cm>;<1.5cm,0cm>:<0cm,1.25cm>::}
