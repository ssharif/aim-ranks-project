...
\begin{proposition}\label{prop:Q-infty-self-intersection}
  The self-intersection $Q_\infty^2$ is ???. 
\end{proposition}
...
\end{array}\right]
  ...
  Combining, we get
  \[
  D_P \cdot P' = -\frac{d-1}{d} - \frac{1}{rd}\right).
  \]
...
\[
\langle P_{00} - Q_\infty, P_{ij} - Q_\infty\rangle = -P_{00} \cdot P_{ij} + P_{00} \cdot Q_\infty + P_{ij} \cdot Q_\infty - Q_\infty^2 + D_P_{00} \cdot P_{ij}.
\]
According to
