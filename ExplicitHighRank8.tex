...
In this section, we compute the height pairing on various sections of our curve. Specifically, we will compute $\langle P_{i,j} - Q_\infty, P_{i', j'} - Q_\infty \rangle$. By bilinearity, it suffices to compute $\langle P_{i,j} - Q_\infty, P_{0,0} - Q_\infty \rangle$ for all $i,j$. In all of the following, we base-extend to the field $K(u)$ where $u^d = t$.
