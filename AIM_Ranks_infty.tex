...
\begin{align*}
  ...
  E_P&: (u = \tau = 0).
\end{align*}
In the above, $\tilde{C_\cdot}$ is the strict transform of the corresponding component with the same multiplicities as before. To compute the multiplicity of the third component $E_P$, we consider the ring
\[
\frac{k[u,v,\tau]}{(u^{r-1}v^{r-1}(v + u\tau)(v + 1) - \tau, T)}
\]
where $T = u\tau$. We wish to determine the multiplicity of the prime ideal $(u,\tau)$, so we may localize. Let $\alpha = v^{r}(v+1)$. Then the ideal in the above ring becomes
\[
(\alpha u^{r-1} - \tau, u\tau).
\]
The first expressions allows us to substitute $\tau = \alpha u^{r-1}$. The second expression then becomes $\alpha u^r$. Therefore the multiplicity of $E_p$ in the special fiber is $r$.
...
\shahed{Problem: If we compute the self-intersection $E_P^2$, we get a nonsensical result. Namely, we use the fact that $E_P$ intersected with the total fiber is zero, and that $E_P$ only intersects $C_1$ and $C_z$. Therefore
  \begin{align*}
    0 &= E_P \cdot (C_1 + rE_p + rC_z) \\
    &= 1 + rE_p^2 + r
  \end{align*}
  from which $E_P^2 = -1 - \frac{1}{r}$. I don't see where I went wrong.}
...
\]
where $z = bw$ and $T = \tau w$. The special fiber is given by $T = 0$, and consists of the components with equation $\tau = b = 0$ and $\tau = w = 0$. The first component is the strict transform of ${C_z}$ and has multiplicity $r$. The second component is the exceptional component, which we call $E_1$. Its multiplicity is computed in a similar manner as that of $E_P$; that is, the dimension of
\[
\frac{k[b,w,\tau]_{(\tau, w)}}{(b^{r-1}w^{r-2}(bw + \tau)(bw^2 + 1) - \tau, \tau w)}
\]
over the residue field $k(b)$. To do this computation, we let $\alpha = b^r(bw^2 + 1)$ and notice that since $r \geq 3$, the first expression in the ideal becomes $\alpha w^{r-1} - \tau$. Substituting for $\tau$ in the second expression, we obtain $\alpha w^r$. Therefore $E_1$ has multiplicity $r$.
