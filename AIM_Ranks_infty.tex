...
\end{equation}
and has coordinates $(w,z,T) = (0,0,0)$. Just as in the case of $P$, we will desingularize at $Q$ recursively; however, the situation is rather more complicated. To start with, we will assume that $r \geq 4$. 
...
We therefore continue by blowing-up chart 2. We do this recursively as follows. For $1 \leq i \leq r-3$, let chart $(2,i)$ be the chart given by
  ...
  \]
  with $w_{r-2}, T_{r-2}$ given as above, and $T = T_{r-2} z^{r-2}$. We blow up as before; that is, construct the charts $(2, r-1)$, $(2, r-2)a$ and $(2, r-2)b$ as above. We now verify that there are two differences: every chart will be regular, and the special fiber for $(2, r-1)$ and $(2,r-1)b$ will be different.
  ...
  \end{itemize}
  One checks that the $T_{r-1}$ partial derivative is zero if and only if $z = 0$ and $w_{r-1} = 1$. But plugging these values into the $z$-derivative yields $1$. Therefore this chart is regular.
  ...
  \end{itemize}
  The argument for regularity is unchanged.
  ...
  \end{itemize}
  This chart is regular because ???

\subsubsection{Dual graph}
\label{sec:dual-graph}

Putting it all together, we obtain the dual graph for $C_k$ pictured in Figure~\ref{fig:superelliptic-dual-graph}.
...
All intersections are transversal. The components $C_1$ and $C_2$ have multiplicity $1$. The $R_i$ each have multiplicity $r$. The $D_i$ and $E_i$ have multiplicity $i$.
...
In our case, all of the geometric intersections are transversal, so we can simplify the above theorem.
\begin{proposition}
  Let $\scd$ be a regular arithmetic surface proper over a discrete valuation ring, and let $C_k$ be the special fiber. Let $C_k^{red}$ be the associated reduced scheme. Suppose that every singular point of $C_k^{red}$ consists of a transverse crossing of two irreducible components. Let $D$ be an irreducible component of $C_k$. Label the remaining irreducible components $D_1, D_2, \dots$. Then
  \[
  D^2 = -\frac{1}{m} \sum m_i n_i
  \]
  where $m$ is the multiplicity of $D$, $m_i$ is the multiplicity of $D_i$, and $n_i$ is the number of intersection points of $D$ and $D_i$.
\end{proposition}
