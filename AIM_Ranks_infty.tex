...
u^rv^{r-1}(v + 1) - 1.
\]
We now observe that this expression equals $-1$ at both $P_1$ and $P_2$. Therefore both points are regular.
...
\begin{align*}
  ...
  E_P&: (z = \pi = 0).
\end{align*}
The intersection $\tilde{C_1} \cap E_P$ occurs when $1 + x = \pi = z = 0$. One checks that this intersection point is in fact $P_1$. The intersection $\tilde{C_z} \cap E_P$ occurs when $x = \pi = z = 0$. But now note that $x = \frac{1}{v}$. Therefore this intersection point is distinct from $P_2$. We call this new point $P_3$. One checks that $P_3$ is \emph{not} regular.
